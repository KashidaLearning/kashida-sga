"""
This block defines a Kashida Staff Graded Assignment. Students are shown a rubric
...
from kashida_sga.constants import (
    ATTR_KEY_ANONYMOUS_USER_ID,
    ATTR_KEY_USER_IS_STAFF,
    ATTR_KEY_USER_ROLE,
    ITEM_TYPE,
)
from kashida_sga.showanswer import ShowAnswerXBlockMixin
from kashida_sga.tasks import (
    get_zip_file_name,
    get_zip_file_path,
    zip_student_submissions,
)
from kashida_sga.utils import (
    file_contents_iter,
    get_default_storage,
...
@XBlock.needs("user")
class KashidaStaffGradedAssignmentXBlock(
    StudioEditableXBlockMixin, ShowAnswerXBlockMixin, XBlock
):
    """
    Kashida Staff Graded Assignment XBlock
    """

    has_score = True
    icon_class = "problem"
    STUDENT_FILEUPLOAD_MAX_SIZE = 100 * 1024 * 1024  # 100 MB

    editable_fields = ("display_name", "points", "weight", "showanswer", "solution")

    display_name = String(
        display_name=_("Display Name"),
        default=_("Kashida Staff Graded Assignment"),
    ...
    staff_score = Integer(
        display_name=_("Score assigned by staff"),
        default=None,
        scope=Scope.settings,
    )
    ...
    annotated_sha1 = String(scope=Scope.user_state, default=None)
    annotated_filename = String(scope=Scope.user_state, default=None)
    annotated_mimetype = String(scope=Scope.user_state, default=None)
    annotated_timestamp = DateTime(scope=Scope.user_state, default=None)
    ...
    def upload_assignment(self, request, suffix=""):
        ...
        if not is_allowed_file(filename):
            raise JsonHandlerError(
                400,
                "Invalid file type. Allowed types: pdf, doc, docx, mp4, mov, mp3, wav, m4a"
            )
